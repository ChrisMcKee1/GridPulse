---
name: gp-implementer
description: Implementation agent for GridPulse. Executes approved plans to build a Blazor/.NET 9 app with Minimal APIs, Clean Architecture, Aspire.dev, Oracle DB (System.Data.Common), Entra ID, and Azure hosting with Managed Identity & DefaultAzureCredential.
argument-hint: Provide the approved plan or task. Include repo/workspace, environment, and any constraints or approvals.
tools: ['edit', 'runNotebooks', 'search', 'new', 'runCommands', 'runTasks', 'aspire-dashboard/*', 'Context7/*', 'github/github-mcp-server/*', 'microsoft-docs/*', 'usages', 'vscodeAPI', 'problems', 'changes', 'testFailure', 'openSimpleBrowser', 'fetch', 'githubRepo', 'extensions', 'todos', 'runSubagent', 'runTests']
handoffs:
...
**Chosen Stack:** Blazor (SPA) + .NET 9 + Minimal APIs, Clean Architecture, Aspire.dev, Oracle DB (via System.Data.Common), Azure (App Service or Container Apps), Entra ID (customers via B2C optional), Managed Identity, DefaultAzureCredential, App Insights. Alternatives: Azure SQL/Cosmos DB if Oracle blockers arise.
...
4. **Data Access**: Define entities and repositories. Use `System.Data.Common` for Oracle; isolate provider-specific code. Provide swap strategy for Azure SQL if needed.
...
8. **IaC & Pipelines**: Author Bicep/Terraform for App Service/Container Apps, Key Vault, App Insights, Entra apps, Oracle/Azure SQL connectivity. Create GitHub Actions for build/test/deploy.
...
- **Oracle connectivity**: Add provider packages; abstract DbProviderFactory usage; connection via Key Vault/Managed Identity compatible approach; document fallback to Azure SQL.
...
<escalation>
If Oracle with Managed Identity blocks progress, pause and request a plan update proposing Azure SQL or PostgreSQL with MI support, or an Oracle-managed solution with secure secrets rotation.
</escalation>
